<%-- 
    Document   : AjaxSearchShopUtensil
    Created on : 19 Nov, 2021, 2:14:27 PM
    Author     : ajula
--%>
...
<%
    ...
    if (request.getParameter("action") != "") {
        ...
        String sqlQry = "SELECT * FROM tbl_utensil d INNER JOIN tbl_utensilbrand db ON d.utslbrnd_id = db.utslbrnd_id where sh_id='" + request.getParameter("id") + "' ";
        String row = "SELECT count(*) as count FROM tbl_utensil d INNER JOIN tbl_utensilbrand db ON d.utslbrnd_id = db.utslbrnd_id where sh_id='" + request.getParameter("id") + "' ";
        System.out.println(sqlQry);
        if (!request.getParameter("brand").equals("")) {

            String brand = request.getParameter("brand");

            sqlQry += "AND d.utslbrnd_id IN('" + brand + "')";
            row += "AND d.utslbrnd_id IN('" + brand + "')";
        }
