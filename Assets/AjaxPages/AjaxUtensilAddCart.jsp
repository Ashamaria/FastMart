<%-- 
    Document   : AjaxUtensilAddCart
    Created on : 19 Nov, 2021, 9:50:08 PM
    Author     : ajula
--%>
...
<%
    ...
    if (Integer.parseInt(rsC.getString("count")) > 0) {
        ...
        if (rs.next()) {
            String sqlQry = "select count(*) as count from tbl_cart where booking_id='" + rs.getString("id") + "' and utsl_id='" + id + "'";
            ResultSet rsc = con.selectCommand(sqlQry);
            rsc.next();
            if (Integer.parseInt(rsc.getString("count")) > 0) {
                out.println("Already Added to Cart");
            } else {
                String insQry = "insert into tbl_cart(booking_id,utsl_id)values('" + rs.getString("id") + "','" + id + "')";
